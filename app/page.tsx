...
function Button({ children, variant = 'primary', href = '#demo' }: { children: React.ReactNode; variant?: 'primary' | 'secondary' | 'ghost'; href?: string }) {
  return <a href={href} className={`btn ${variant}`}><span>{children}</span><ArrowRight size={16} /></a>;
}
  ...
  return <main>
    <header className="nav-wrap">
      <nav className="nav container">
        <a href="#top" className="brand"><span className="brand-mark"><CalendarDays size={18}/></span><span>TURNOS</span></a>
        ...
        <div className="nav-actions"><a className="login" href="#cta">Ingresar</a><Button href="#planes">Probar TURNOS</Button></div>
    ...
    <section id="top" className="hero">
      <div className="container hero-grid">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><Button href="#planes">Probar TURNOS</Button><Button variant="secondary" href="#demo">Ver cómo funciona</Button></div>
    ...
    <section className="logos"><div className="container logo-row"><span>Hecho para equipos que trabajan con agenda</span><i/><span>Simple para tus clientes</span><i/><span>Potente para tu negocio</span></div></section>
    ...
    <section id="negocios" className="section verticals"><div className="container"><div className="section-head"><div><span className="kicker">PARA TU NEGOCIO</span><h2>Una base. Distintos negocios.</h2></div><p>TURNOS se adapta al flujo de servicios donde el tiempo y la agenda importan.</p></div><div className="vertical-grid">{verticals.map(({icon:Icon,title,text})=><div className="vertical-card" key={title}><Icon size={25}/><h3>{title}</h3><p>{text}</p><a href="#planes">Conocer TURNOS <ArrowRight size={15}/></a></div>)}</div></div></section>

    <section id="demo" className="section demo"><div className="container demo-grid"><div><span className="kicker">DEMO</span><h2>Tu cliente reserva. Vos ves qué pasa.</h2><p>Una vista de producto que hace tangible la propuesta: disponibilidad, reservas y clientes en una experiencia simple.</p><div className="demo-list"><span><Check/> Reserva sin llamadas</span><span><Check/> Agenda siempre actualizada</span><span><Check/> Datos listos para crecer</span></div><Button href="#planes">Crear mi sistema de turnos</Button></div><div className="phone-wrap"><div className="phone"><div className="notch"/><div className="phone-top"><span>9:41</span><span>● ● ●</span></div><div className="phone-content"><small>RESERVÁ TU TURNO</small><h3>Elegí un servicio</h3><div className="service-selected"><div className="service-icon">✦</div><div><b>Consulta</b><span>45 minutos</span></div><Check size={17}/></div><h4>Elegí un horario</h4><div className="days"><b>Hoy<br/><span>14</span></b><span>Jue<br/><span>15</span></span><span>Vie<br/><span>16</span></span><span>Sáb<br/><span>17</span></span></div><div className="slots"><span>09:30</span><span>10:15</span><b>11:00</b><span>11:45</span></div><button>Confirmar reserva <ArrowRight size={14}/></button></div></div></div></div></section>
    ...
    <section className="section faq"><div className="container faq-grid"><div><span className="kicker">PREGUNTAS FRECUENTES</span><h2>Lo importante, antes de empezar.</h2><p>Sin letra chica sobre capacidades que todavía no existen.</p></div><div>{faqs.map(([q,a],i)=><div className={`faq-item ${openFaq===i?'active':''}`} key={q}><button onClick={()=>setOpenFaq(openFaq===i?null:i)}><span>{q}</span><ChevronDown size={18}/></button>{openFaq===i && <p>{a}</p>}</div>)}</div></div></section>

    <section id="cta" className="final-cta"><div className="container"><div className="cta-card"><div className="cta-orb orb-a"/><div className="cta-orb orb-b"/><span className="kicker light">EMPEZÁ HOY</span><h2>Tu agenda puede trabajar<br/>mientras vos trabajás.</h2><p>Convertí reservas en clientes y clientes en recurrencia con una experiencia hecha para tu negocio.</p><div className="hero-actions"><Button href="#planes">Crear mi sistema de turnos</Button><a className="cta-link" href="#funcionalidades">Ver funcionalidades <ArrowRight size={16}/></a></div></div></div></section>
...
function Price({name,desc,features,featured=false}:{name:string,desc:string,features:string[],featured?:boolean}) { return <div className={`price-card ${featured?'featured':''}`}>{featured&&<span className="popular">Más elegido</span>}<h3>{name}</h3><p>{desc}</p><div className="price">Consultar<span> según configuración</span></div><Button href="#cta">Elegir {name}</Button><div className="divider"/>{features.map(f=><div className="price-feature" key={f}><Check size={15}/>{f}</div>)}</div> }
